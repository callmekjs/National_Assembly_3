...
# 재순위 모델 — 2026-08-07 **오후에 sol → luna 로 교체**.
#
# 오전 비교(옛 검색 기준)에서는 sol 이 최고였다:
#   mini 0.697 / terra 0.720 / luna 0.751 / sol 0.768 / sol+low 0.778
# 그런데 같은 날 키워드 검색에 IDF 가중치를 넣고 나서 다시 재니 뒤집혔다:
#   sol nDCG@5 0.755 / **luna 0.762** (strict R@5 는 둘 다 1.000, 유형별 전부 만점)
# 검색이 좋은 후보를 이미 위로 올려주니 재순위 모델의 힘이 덜 중요해진 것이다.
# **앞단을 고치면 뒷단의 최적값이 바뀐다** — 한 번 잰 채택 결정도 유효기간이 있다.
#
# 비용 차이가 결정적이다. 재순위는 후보 30개를 매 질의마다 통째로 보내 입력이
# ~8,400토큰이라, 모델 단가가 그대로 질의 비용이 된다:
#   sol($5/$30) $0.047/질의 → luna($0.20/$1.20) $0.002/질의 — **25배**
# 전체 질의 비용 $0.061 → $0.015 이 되어 $1/일 상한에서 14회 → 66회로 늘어난다.
# 답변 생성(answer.MODEL)은 gpt-5.6-terra 로 별개다.
_MODEL = os.environ.get("RERANKER_MODEL") or "gpt-5.6-luna"
