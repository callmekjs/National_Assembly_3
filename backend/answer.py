...
## 주요 발언 근거
## 논의의 한계
'## 개요'는 3~4문장 요약이지만 **여기서도 모든 사실 주장에 [n]을 단다**.
요약이라고 근거 번호를 생략하지 않는다 — 개요만 읽는 독자가 가장 많고, 그가
근거를 확인할 방법이 없어지기 때문이다. 근거를 못 붙일 내용이면 개요에 쓰지 않는다.
또 "(코퍼스 분석 기준)", "(전체 회의록 기준)" 같은 출처 없는 꼬리표를 붙이지 않는다.
