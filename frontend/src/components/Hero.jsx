...
//
// 차이의 원인은 검색 성능이 아니라 **근거의 부재**였다. 라벨된 핵심 근거(등급2)
// 142개 중 73개만 축소본에 있었다(48.6% 누락). 없는 것을 찾으라고 시킨 셈이다.
// 그래도 배포본 숫자를 앞에 둔다 — 방문자의 경험을 설명하는 건 이쪽이고,
// 변명은 그 다음이다.
//
// 2026-08-07 저녁: 임베딩을 512차원으로 줄여 같은 용량에 청크를 2.8배 담았고
// (18,001 → 50,726), 그 결과 **15/18 → 17/18**. 회복된 두 문항이 예측과 정확히
// 일치한다 — n001(근거 0/2 → 2/2)·n010(4/8 → 7/8). 남은 n014 는 근거의 96%가
// 여전히 코퍼스 밖이라 용량으로는 해결되지 않는다.
//
// 값은 사용자 요청으로 백분율(`94.4%`)로 쓴다. 다만 **표본 크기를 잃지 않는다** —
// `94.4%` 만 있으면 18문항이라는 사실이 숨고, "몇 문제로 쟀나"는 반드시 나오는
// 질문이다. 화면에 없으면 숨긴 것처럼 보인다. 그래서 분모를 설명줄로 옮겼다
// (`18문항 중 17개`). 큰 숫자는 눈에 띄고 조건은 바로 밑에 있다 — 둘 다 만족한다.
//
// 2026-08-08: `답변 정확도` 도 교체했다. 직전까지 `89.3%(75문항 사람 검수)` 였는데,
// **그 채점표는 "답이 맞았나"를 묻지 않는다.** 4기준(근거 밖 창작·인용 정확성·정당
// 분류·거절 여부) 중 어느 것도 사실 여부를 보지 않아, 검색이 엉뚱한 회의록을
// 가져와도 얌전히 요약하고 `[n]` 만 잘 달면 통과였다 — progress.md 가 이를
// **"옛 R@5 0.983 과 같은 종류의 동어반복"** 이라 적어 두고도 화면은 그대로였다.
//
// 대체값은 `report_content_eval.py` 의 qa 모드 결과다(16/18). 이쪽은 **사람이
// 20/20 검토한 정답지와 대조**하고, 정답지는 시스템 검색을 쓰지 않고 만들어져
// 검색이 틀리면 점수가 떨어진다. 숫자는 89.3 → 88.9 로 거의 같지만 **재는 대상이
// 다르다** — 같은 값이라고 같은 주장이 아니다.
//
// 이것으로 첫 화면의 지표 3개(검색·답변·분량)가 전부 "무엇을 잰 숫자인지"
// 설명줄에 드러난다. 하루에 같은 유형의 결함을 세 번 고쳤다.
const METRICS = [
  { label: '회의록 분량', value: '42만 발언', sub: '22대 국회 2024-05 ~ 2026-06' },
  { label: '근거 찾기', value: '94.4%', sub: '축소본 기준 18문항 중 17개 · 전체 데이터로는 18/18 · 한계는 README' },
  { label: '답변 정확도', value: '88.9%', sub: '18문항 중 16개 · 사람 검토 정답지와 대조 · 한계는 README' },
