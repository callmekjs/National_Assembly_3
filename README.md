...
22대 국회 회의록 767건(발언 42만 건)을 근거로 **정책 쟁점과 여야 입장 차이**를
분석하는 GovTech RAG 서비스. 모든 문장에 근거 번호를 달고, 회의록에 없으면
지어내지 않고 "확인할 수 없습니다"라고 답한다.

**▶ 데모: https://national-assembly-3.vercel.app**

> 무료 요금제로 배포돼 있어 **첫 요청이 30초 이상** 걸릴 수 있다(15분 유휴 시
> 서버가 잠든다, 실측 33.9초). 한 번 깨면 이후는 즉시 응답한다.
> 검색 대상은 전체 회의록이 아니라 [축소 코퍼스](#알려진-한계)다.
...
| 화면 | 하는 일 |
|------|---------|
| **질의** | 질문 → 회의록 근거를 찾아 답변 + 출처(발언자·정당·회의일·PDF 쪽수) |
| **쟁점 분석** | 24개 쟁점별 월별 발언 추이 · 정당별 입장 분포 · 정권교체 시점 배지 |
| **의원 프로필** | 발언 통계 · 시점별 여야 이력 · 쟁점별 입장 |

답변은 두 모드다. **간단 답변**(qa)은 짧게, **정책 브리핑**(report)은 개요·쟁점·
전망을 갖춘 문서 형태로 낸다.
...
RAG 데모는 흔하다. 이 프로젝트가 신경 쓴 것은 **답이 그럴듯해 보이는 것과
실제로 근거가 있는 것을 구분하는 일**이다.

**1. 근거를 못 대면 거절한다**
답변마다 근거 연결 상태를 등급으로 표시한다 — `FULL`(전부 연결) /
`PARTIAL`(일부만) / `REFUSED`(근거 부족) / `NONE`(검색 0건).
실측: 코퍼스 밖 인물을 물으면 지어내지 않고 "확인할 수 없습니다"로 답한다.
검색이 0건이면 **LLM 을 호출하기 전에** 끊어 비용도 쓰지 않는다.

**2. 인용 번호를 검증한다**
답변의 `[n]`이 실제 출처 범위 안인지 확인하고, 범위 밖이면 `invalid_citations`로
표시한다. 화자·정당·날짜 귀속의 형식 일치도 규칙 7종으로 검사한다.

**3. 자기 평가셋의 결함을 공개한다**
초기 지표(`R@5 0.983`)는 **폐기했다.** 정답 판정이 "결과에 질문 키워드가 있는가"
였는데 키워드 검색 축이 같은 연산을 하는 동어반복이었기 때문이다. 잣대를 다시
쓰고 [알려진 한계](#알려진-한계)에 남은 문제까지 적었다.
...
```mermaid
flowchart TD
    Q["질문"] --> P["질문 파싱<br/>위원회·기간·인물 필터 추출"]
    P --> K["키워드 검색<br/>pg_trgm + IDF 가중"]
    P --> V["벡터 검색<br/>pgvector HNSW"]
    K --> F["RRF 융합"]
    V --> F
    F --> R["LLM 재순위<br/>상위 30건 재정렬"]
    R --> G{"검색 결과가<br/>있는가"}
    G -- 없음 --> N["확인할 수 없습니다<br/>LLM 미호출 · 비용 0"]
    G -- 있음 --> A["답변 생성<br/>근거 블록 + 인용 지시"]
    A --> C["인용 검증<br/>범위 밖 번호 검출"]
    C --> S["Grounding 판정<br/>FULL · PARTIAL · REFUSED"]
    S --> O["답변 + 출처 패널<br/>원문·앞뒤 맥락·PDF 쪽수"]
```

**데이터 준비**는 별도 파이프라인이다 —
PDF 767건 → 페이지 추출(41,571쪽) → 정규화 → 발언자 턴 파싱(418,758턴) →
품질 게이트 → 정책 메타데이터 → 청크(419,882) → 임베딩 → PostgreSQL.
각 단계에 품질 게이트가 있고, 실패하면 다음 단계로 넘어가지 않는다.

---

## 측정 결과

**이 표의 숫자를 인용하려면 [알려진 한계](#알려진-한계)를 함께 읽어야 한다.**
자체 제작 평가셋이고 공인 벤치마크가 아니다.

| 무엇을 쟀나 | 결과 | 조건 |
|---|---|---|
| 상위 5건 안에 근거가 있는가 | **17/18** | 배포본(축소 코퍼스) · 전체 코퍼스는 18/18 |
| 상위 10건까지 넓히면 | **18/18** | 배포본 · report 모드가 보는 범위 |
| 순위 품질 (nDCG@5) | 0.655 | 배포본 · 전체 코퍼스는 0.762 |
| 답 없는 질문을 거절하는가 | 2/2 | 지어내지 않고 거절 |
| 답변 정확성 | 67/75 (89.3%) | 사람 검수 · 측정 방식에 한계 있음 |
| 테스트 | 153개 통과 | 백엔드 + 프론트 + CI |

**배포 환경 실측** (2026-08-07, 축소 코퍼스 18,001청크 기준)

| 항목 | 값 |
|---|---|
| 하이브리드 검색 | 5.2초 |
| 질의 1건 (qa) | 13초 · $0.016 |
| 콜드스타트 | 33.9초 (15분 유휴 후 첫 요청) |
| 동시 요청 10건 | 전부 성공 |
...
**3. 동시 처리 한계가 낮다 — 단, `/query` 기준이다**
DB 커넥션 풀 5개인데 **`/query` 1건이 키워드·벡터 두 축을 병렬 실행하며 커넥션 2개를
동시 점유**한다 → 실질 동시 처리 2~3건. 초과분은 최대 10초 대기 후 오류. 데모 규모
전제의 설정이며 수평 확장은 범위 밖.

단일 검색 엔드포인트는 커넥션을 1개만 쓰므로 이 한계가 적용되지 않는다 —
2026-08-07 배포 실측에서 `/search/keyword` 동시 10건이 전부 성공했다(3.9초).
이 절이 예전에 `/query` 기준임을 밝히지 않아 실제보다 비관적으로 읽혔다.
...
| | 청크 | 차원 | strict@5 | nDCG@5 | MRR@10 |
|---|---|---|---|---|---|
| 전체 코퍼스(로컬) | 420,378 | 1536 | 18/18 | 0.762 | 0.870 |
| 배포 축소본 (아침) | 6,692 | 1536 | 15/18 | 0.468 | 0.738 |
| 배포 축소본 (오후) | 18,001 | 1536 | 15/18 | 0.522 | 0.743 |
| **배포 축소본 (현재)** | **50,726** | **512** | **17/18** | **0.655** | **0.911** |

차이의 원인은 검색 성능이 아니라 **근거의 부재**다. 라벨된 핵심 근거(등급2)
142개 중 73개만 축소본에 있었다(**48.6% 누락**). 누락률이 높은 문항(n013 87.8%,
n014 96.4%, n018 90.9%)이 점수가 낮은 유형(date_based 0.50, person 0.80)과
그대로 겹친다 — 못 찾은 게 아니라 없는 것을 찾게 시킨 결과다.

1536차원에서는 코퍼스를 2.7배(6,692 → 18,001)로 늘려도 strict@5 가 15/18 로
그대로였다. 근거가 회의 안에서 넓게 퍼져 있어(±60 까지 가야 100%) 인접 창을
조금 넓히는 것으로는 회수되지 않고, HNSW 를 유지하면 500MB 안에서 ±1 이 한계였다.

**임베딩을 512차원으로 줄여 풀었다.** 인덱스 크기가 차원에 묶여 있으므로,
인덱스를 지키면서 자리를 벌려면 벡터를 작게 만드는 수밖에 없다. 같은 용량에
청크를 2.8배(18,001 → 50,726) 담았고 **strict@5 가 17/18 로 올랐다.**

회복된 두 문항이 사전 예측과 정확히 일치한다 — n001(핵심 근거 0/2 → 2/2),
n010(4/8 → 7/8). 남은 n014 는 근거의 96%가 여전히 코퍼스 밖이라 **용량으로는
해결되지 않는다**(±10 까지 넓혀도 6/13).

차원 축소의 대가는 실측했다. **의미 검색 축만 보면 nDCG@5 가 29% 떨어진다**
(0.150 → 0.107). 그런데 최종 파이프라인에서는 키워드 축과 재순위가 흡수해
같은 코퍼스 기준 -5% 에 그친다. **하이브리드 구조라서 성립하는 거래**이고,
의미 검색 하나만 쓰는 설계였다면 하지 못했을 선택이다.

잘라내기는 API 에 `dimensions=512` 로 재요청한 것과 사실상 같다(코사인 0.999999
실측) — `text-embedding-3` 계열이 중요한 정보를 앞쪽에 모으도록 훈련돼 있어서다.
재생성 비용은 0원이었다.
...
| 배포 | **Vercel**(프론트) + **Render** 싱가포르(백엔드) + **Supabase** 서울(DB) — 전부 무료 요금제 |
| 테스트·CI | pytest 153개 + vitest + oxlint, GitHub Actions (pip-audit·npm audit) |
...
- [x] **4단계: GovTech 배포 버전 — 완료 (2026-08-07)**
  - [x] A 방어선: rate limit(일반 60/분·LLM 5/분)·일별 비용 상한 $3·재순위 출력 상한
  - [x] B 실배포: Vercel(프론트) + Render 싱가포르(백엔드) + Supabase 서울(DB)
  - [x] 배포 스모크 13종 — 결함 2건 발견·수정 (공백 입력 502, 첫 화면이 배포본과 다른
        코퍼스 수치를 노출). 상세는 `docs/progress.md` "4단계-B 실배포 완료"
