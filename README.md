...
| 무엇을 쟀나 | 결과 | 조건 |
|---|---|---|
| 상위 5건 안에 근거가 있는가 | **15/18** | 배포본(축소 코퍼스) · 전체 코퍼스는 18/18 |
| 순위 품질 (nDCG@5) | 0.522 | 배포본 · 전체 코퍼스는 0.762 |
...
| | 청크 | strict@5 | nDCG@5 | MRR@10 |
|---|---|---|---|---|
| 전체 코퍼스(로컬) | 420,378 | 18/18 | 0.762 | 0.870 |
| 배포 축소본 (확대 전) | 6,692 | 15/18 | 0.468 | 0.738 |
| **배포 축소본 (현재)** | **18,001** | **15/18** | **0.522** | **0.743** |
...
**코퍼스를 2.7배로 늘렸지만 strict@5 는 그대로다.** 근거가 회의 안에서 넓게
퍼져 있어(±60 까지 가야 100%) 인접 창을 넓히는 것으로는 회수되지 않는다.
무료 한도 500MB 안에서 HNSW 를 유지하며 넣을 수 있는 최대가 ±1(18,001청크)이고,
근거 회수율은 47.2% → 49.3% 로 2.1%p 오를 뿐이다. 순위 품질(nDCG@5)은
0.468 → 0.522 로 개선됐고 지연은 오히려 줄었다(하이브리드 10.2초 → 5.2초).

**무료 한도 안에서 이 방향의 개선은 사실상 소진됐다.** 다음 지렛대는 임베딩
차원 축소다 — `text-embedding-3-small` 을 1536 → 512 차원으로 줄이면 저장·인덱스가
1/3 이 되어 같은 용량에 3배를 담을 수 있다(재생성 비용 약 $0.22). 다만 차원 축소가
검색 품질에 주는 영향을 **배포와 같은 조건에서** 먼저 재야 한다.
