...
22대 국회 회의록을 근거로 **정책 쟁점과 여야 입장 차이**를 분석하는 GovTech RAG
서비스. 모든 문장에 근거 번호를 달고, 회의록에 없으면 지어내지 않고
"확인할 수 없습니다"라고 답한다.

**▶ 데모: https://national-assembly-3.vercel.app**

**▶ [포트폴리오 요약과 최신 검증 결과](docs/llm_loop/PORTFOLIO.md)**

### 최신 품질 상태 — 2026-08-19

신규 독립 블라인드 100문항의 공식 최종 결과는 **81점**이다. 검색 RRF@10 96.67%,
최종 문맥 Recall@5 100%, 인용 정확도 96.67%, 답변 불가 거절 100%는 통과했지만,
의미 정확도 90%와 치명 오류 0건 기준은 통과하지 못했다. 따라서 이 저장소는 결과를
“90점 완성품”으로 표시하지 않는다.

최종평가 뒤 치명 오류 4건의 일반 원인을 수정하고 전체 회귀 **336개**를 통과했지만,
공개된 블라인드를 다시 돌려 점수를 올리지 않았다. 공식 81점과 사후 보완을 분리한 이유와
실패 내역은 [최종 보고서](docs/llm_loop/FINAL_REPORT.md)에 기록했다.
...
## 먼저 알아야 할 것 — 데모는 축소본이다

**이 README 의 모든 수치는 별도 표기가 없으면 배포본(축소 코퍼스) 기준이다.**

| | 만든 것 (로컬) | **배포한 것 (데모)** |
|---|---|---|
| 회의록 | 767건 · 41,571쪽 | 767건 (메타데이터 전량) |
| 발언 청크 | **420,004** | **50,726** (12%) |
| 임베딩 차원 | 1536 | **512** |
| 저장 용량 | 약 9.6GB | **362MB** |

**왜 줄였나 —** 무료 인프라의 저장 한도 때문이다. Supabase 무료 플랜이 **500MB**
이고, 회의록 전체를 임베딩과 인덱스까지 올리면 9.6GB 다. 20분의 1로 줄여야 했다.

**어떻게 줄였나 —** 무작위 표본이 아니라 **24개 정책 쟁점에 연결된 발언과 그 앞뒤
맥락**을 남겼다. 쟁점 분석이 이 서비스의 핵심이라 그 축을 살린 것이다. 임베딩은
1536차원 벡터의 앞부분을 잘라 512차원으로 썼다 — 같은 용량에 2.8배를 담기 위해서다
(잘라내기가 API 재요청과 사실상 같다는 것은 [실측](#알려진-한계)했다).

**무엇을 잃었나 —** 라벨된 정답 근거의 3분의 1이 코퍼스 밖이다. 그래서 같은 평가셋에서
로컬은 18/18, 배포본은 17/18 이다. **검색 성능의 차이가 아니라 자료의 차이**다.
자세한 수치와 대가는 [알려진 한계](#알려진-한계)에 있다.
...
**데이터 준비**는 별도 파이프라인이다 (전량 로컬에서 수행) —
PDF 767건 → 페이지 추출(41,571쪽) → 정규화 → 발언자 턴 파싱(419,024턴) →
품질 게이트 → 정책 메타데이터 → 청크(420,004) → 임베딩 → PostgreSQL.
각 단계에 품질 게이트가 있고, 실패하면 다음 단계로 넘어가지 않는다.
**배포본은 이 결과물에서 쟁점 관련분만 추려 옮긴 것**이다.
...
| 무엇을 쟀나 | **배포본** | (참고) 전체 코퍼스 |
|---|---|---|
| 상위 5건 안에 근거가 있는가 | **17/18** | 18/18 |
| 상위 10건까지 넓히면 | **18/18** | 18/18 |
| 순위 품질 (nDCG@5) | **0.655** | 0.762 |
| 답 없는 질문을 거절하는가 | **2/2** | 2/2 |

**차이는 검색 성능이 아니라 자료의 유무다.** 라벨된 핵심 근거 142개 중 축소본에
있는 것이 96개다(**46개 누락, 32.4%**) — 없는 것을 찾으라고 시킨 결과다.

답변 품질은 코퍼스와 무관하게 잰다(모범답안 대조, 전체 코퍼스 기준):

| 무엇을 쟀나 | 결과 |
|---|---|
| 답변이 맞았는가 (간단 답변) | **16/18 (88.9%)** · 환각 1건 |
| 〃 (정책 브리핑) | 14/18 (77.8%) · 환각 3건 — 길게 쓸수록 지어낼 여지가 는다 |
| 테스트 | 153개 통과 (백엔드 + 프론트 + CI) |

**배포 환경 실측** (2026-08-08, 축소 코퍼스 50,726청크 · 512차원 기준)
...
- **옛 답변 지표(`89.3%`, 75문항)도 폐기했다.** 그 채점표의 4기준(근거 밖 창작·인용
  정확성·정당 분류·거절 여부) 중 어느 것도 **"질문에 대한 답이 맞았나"를 묻지 않는다.**
  검색이 엉뚱한 회의록을 가져와도 얌전히 요약하고 `[n]` 만 잘 달면 통과였다 —
  폐기한 `R@5 0.983` 과 같은 종류의 동어반복이다. 지금 값(16/18)은 정답지 대조라
  검색이 틀리면 떨어진다. **숫자가 89.3 → 88.9 로 거의 같지만 재는 대상이 다르다.**
...
**6. 배포본은 축소 코퍼스다 — 부득이한 선택이었다**

로컬에서는 회의록 420,004청크 전량으로 돌아간다. 하지만 **무료 인프라로 공개
배포하려면 20분의 1로 줄여야 했다** — Supabase 무료 플랜의 저장 한도가 500MB 인데
전체 코퍼스는 임베딩·인덱스까지 9.6GB 다.

무작위로 줄이지 않았다. **24개 정책 쟁점에 연결된 발언과 앞뒤 맥락**을 남겨
이 서비스의 핵심 축을 지켰다. 그래서 쟁점·의원 분석은 정상 동작하지만,
쟁점과 무관한 질문은 근거를 못 찾을 수 있다.

용량 제약으로 trgm 인덱스도 생성하지 않는다. **로컬에서 측정한 검색 지연 수치는
배포 환경에 적용되지 않는다.** 프로필·타임라인 통계도 축소본 기준이다(화면에 명시).

2026-08-07~08 에 같은 평가셋을 두 코퍼스에 각각 돌려 차이를 실측했다.
(로컬 행의 420,378 은 **그때 잰 값**이다 — 이후 파서 수정으로 현재는 420,004 다.
측정 기록이므로 당시 수치를 그대로 둔다.)

| | 청크 | 차원 | strict@5 | nDCG@5 | MRR@10 |
|---|---|---|---|---|---|
| 전체 코퍼스(로컬, 측정 당시) | 420,378 | 1536 | 18/18 | 0.762 | 0.870 |
...
142개 중 아침 축소본에는 73개만 있었다(**48.6% 누락**). 근거를 늘린 현재
축소본은 96개다(**32.4% 누락**) — 점수가 15/18 에서 17/18 로 오른 것이
검색을 고쳐서가 아니라 **없던 근거를 넣어서**임을 보여준다. 누락률이 높은 문항(n013 87.8%,
...
| 테스트·CI | pytest 154개 + vitest + oxlint, GitHub Actions (pip-audit·npm audit) |
...
| GET | `/search/vector` | 벡터(의미) 검색 — **로컬 디버그 전용**, `ENABLE_DEBUG_ENDPOINTS=1` | ⚙️ |
| GET | `/search/hybrid` | 하이브리드 검색 — **로컬 디버그 전용**, `ENABLE_DEBUG_ENDPOINTS=1` | ⚙️ |
...
| DB | Supabase `aws-1-ap-northeast-2` (서울) | **362MB / 500MB** · 50,726청크 · 512차원 |
...
│   ├── parsed/           # turns.jsonl (767개, 419,024턴)
│   ├── enriched/         # enriched_turns.jsonl (767개)
│   ├── chunks/           # chunks_v1.jsonl (767개, 420,004청크)
...
- [x] ETL-3: 발언자 턴 파싱 (419,024턴, parser v1.3)
- [x] ETL-4: turns quality gate (767/767 PASS)
- [x] ETL-5: 정책 도메인 enrichment (policy_enricher_v1)
- [x] ETL-6: RAG 청크 생성 (420,004청크)
- [x] ETL-7: PostgreSQL 적재 (9 committees / 767 meetings / 2,276 speakers / 420,004 chunks)
- [x] ETL-8: OpenAI 임베딩 생성 (text-embedding-3-small + HNSW)
  — 현재 419,422벡터. v1.3 재처리분 582청크는 재임베딩 대기
