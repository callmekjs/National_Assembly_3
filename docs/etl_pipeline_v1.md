...
- 모델: `text-embedding-3-small` (1536차원)
  - **배포본은 512차원이다.** 여기서 만든 1536차원 벡터를 `make_deploy_corpus.py` 가
    앞에서 잘라 정규화해 보낸다(무료 DB 500MB 제약 — 같은 용량에 2.8배를 담는다).
    잘라내기는 API 에 `dimensions=512` 로 재요청한 것과 사실상 같아(코사인 0.999999)
    **재생성하지 않는다.** 검색 코드는 저장된 벡터에서 차원을 읽어 맞춘다
    (`search_vector.embedding_dims`) — 상수로 박으면 로컬·배포 중 한쪽이 깨진다.
