...
- **검증 규칙이 죽어도 통과처럼 보인다** (`verification.py:556`) — 규칙이 예외로 죽으면
  `detail["errors"]` 에 이름만 남고 flag 는 안 붙는다. `main.py` 는 flags 만 보므로
  **"검사해서 문제없음"과 "검사가 죽어서 못 함"이 구별 불가**하고 알림도 없다.
  위 3번과 같은 계열. 20분.
- **평가 잣대 재작성** (`retrieval_eval.py:97,114`) — 등급 라벨(0/1/2) qrels + nDCG.
  설계는 합의됨: 5축 풀링 → LLM 심판(앞뒤 맥락 포함) → 의심 건만 사람 검수 →
  strict/loose 병기. 3시간 + 검수 30~60분. **한계 명시 필수**: 풀링 밖 근거는 라벨이
  없으므로 결과는 "recall" 이 아니라 **"pooled recall"**.
...
1. **죽은 검증규칙 은폐 (20분)** — 위 3번과 같은 "거짓 신호" 계열인데 이것만 남았다.
2. **평가 잣대 재작성 (3시간 + 검수)** — 선행조건이 해소돼 이제 착수 가능. 이걸 끝내야
   청킹·리랭킹·모델 A/B 를 **측정할 수 있다**.
