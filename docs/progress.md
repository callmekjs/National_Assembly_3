...
최종 업데이트: 2026-07-08 (POL-3 쟁점 사전 + 매핑 v1.2 + 2단 등급화 진행 — 마감 중, "POL-3 후속" 참조)
...
| POL-3 | 쟁점 사전 구축 | 이슈 정의 방식 **결정 지점**: LLM 클러스터링 vs 수동 시드 사전(하이브리드 검색으로 청크 확장). 22대 주요 이슈 20~30개 + 이슈↔청크 매핑 | 이슈 목록 + 매핑, 무작위 스팟체크 통과 | 🔶 마감 중 (core 게이트 재측정 잔여 — 후속 기록 참조) |
...
- **잔여 (다음 세션 — 재개 지점)**:
  1. `issue_tier_pass.py` 잔여 이슈 실행 (완료 여부는 DB `judge='llm_relevant'` 잔존으로 판별)
  2. core 표본 스팟체크 + 판독 → **core 게이트(≥90%) 측정**
  3. 판독 근거(O/X) 보존 커밋 (최종리뷰 Important — "90.8% 재현 불가" 재발 방지)
  4. progress.md 마감 + 로드맵 POL-3 ✅ 전환 + 최종 리뷰어 증분 재확인
- 오늘 추가 비용: v1.2 재매핑 ~$9 + 등급화 진행분 ~$1 — **POL-3 누적 ~$21**
