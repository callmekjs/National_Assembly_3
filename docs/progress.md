...
최종 업데이트: 2026-08-07 (**4단계-B 실배포 완료** — Vercel+Render+Supabase 연결,
스모크 13종 통과, 결함 2건 발견·수정. 잔여: 답변 평가셋 75문항 재실행)
...
> **정산 (2026-08-07 종료 시점)** — 아래 목록은 항목 28개다(예전엔 제목이 "26건"
> 이었으나 목록과 맞지 않았다. 목록이 맞다). 상태: **완료 11 · 부분 1 · 미착수 16**.
> 부분: `pre_gate` — 측정된 실패는 막혔으나 코드는 그대로. 항목 설명 참조.
...
- ~~**평가 잣대 재작성**~~ → ✅ **해소 (2026-08-06)**. 20문항 평가셋 v2 + 등급 라벨
  qrels + `retrieval_eval_v2.py`. 사람이 20/20 검토. 아래 "평가 잣대 재작성 완료" 절.
...
- **`pre_gate` 무발화** (`grounding.py:75`) — 🔶 **부분 (2026-08-07, `e0a329c`)**.
  측정된 실패 2건은 막혔지만 **`pre_gate` 코드는 그대로다.** 고친 것은 앞단(질문 해석)
  이라, 필터가 걸려 검색이 0건이 되면 기존 `if not hits: return "NONE"` 이 발동하는
  경로를 열어준 것뿐이다. `has_keyword` 가 하나라도 있으면 통과시키는 로직은 남아 있어,
  **위원회·날짜 언급이 없는 질문이 무관한 본문에 키워드 1건만 걸려도 여전히 통과한다.**
...
- ~~**페이지네이션이 행을 중복·누락**~~ → ✅ **해소 (2026-08-07, `a00ff08`)**.
  `/meetings` 에 `m.source_id DESC`, `/speakers` 에 `s.name DESC` 타이브레이커 추가.
- ~~**`/health` 가 매 호출 42만 행 `count(*)` 2회** (`main.py:177`)~~ → ✅ **해소 (2026-08-07)**. pg_class.reltuples 근사, 통계 없으면 실집계 폴백.
  `pingHealth()` 를 부른다. `reltuples` 근사나 캐시로 충분. 15분.
- ~~**CORS 기본값이 개발용** (`main.py:58`)~~ → ✅ **해소 (2026-08-07)**. 미설정 시 경고 로그와 함께 개발 기본값.
  전부 CORS 에러인데 코드는 조용히 localhost 기본값으로 간다. 런북에는 있다. 10분.
- **정당 캐시가 영구** (`party.py:69`) — `members` 갱신해도 재시작 전엔 반영 안 됨.

**운영·마이너**

- ~~**`/feedback` 무인증** (`main.py:501`)~~ → ✅ **해소 (2026-08-07)**. 로그인 사용자는 본인 것만, 익명은 최초 1회만. 실패는 404 통일.
...
- ~~**재순위 비용을 답변 모델 단가로 계산**~~ → ✅ **해소 (2026-08-07, `a00ff08`)**.
  **경고가 현실이 됐다** — 8/6 에 리랭커를 `gpt-5.6-sol` 로 바꾸자 장부가 조용히
  틀어졌고, 이 항목을 처리하러 갔다가 발견했다. `PRICES` 표 + 보수적 폴백.
- ~~**`reranker` 만 `print` 사용**~~ → ✅ **해소 (2026-08-07, `a00ff08`)**. `logging` 통일.
- ~~**설정 불일치**~~ → ✅ **해소 (2026-08-07, `a00ff08`)**. `.env.example` 기본 1 +
  `RERANKER_MODEL`·`RERANKER_EFFORT` 명시. 리랭커 채택은 새 잣대(20문항 v2)로
  재측정해 확인했다 — nDCG@5 ON 0.778 / OFF 0.326.
- ~~**브라우저 뒤로가기 무효** (`App.jsx:56`)~~ → ✅ **해소 (2026-08-07)**. pushState + popstate 리스너. 브라우저로 실제 확인.
  없어 탭 이동 후 뒤로가기가 사이트 밖으로 나간다. 15분.
- ~~**에러 바운더리 없음** (frontend)~~ → ✅ **해소 (2026-08-07)**. ErrorBoundary.jsx 신설. 본문만 감싸 헤더·탭은 살리고 resetKey 로 탭 이동 시 자동 복구. Hero 를 일부러 던지게 해 실제 확인.
...
## 평가 잣대 재작성 완료 (2026-08-06) — 처음으로 믿을 수 있는 검색 점수

감사에서 "R@5 0.983 은 동어반복"으로 판정된 잣대를 폐기하고 새로 만들었다.
**옛 63문항 평가셋과 그 라벨은 전량 삭제했다.**

### 왜 전부 버렸나

옛 평가셋의 병은 두 가지였고 둘 다 부분 수정으로 안 되는 종류였다.

1. **18문항이 질문이 아니라 조각이었다** — "유영하 위원의 질의", "가상자산 이용자 보호 방안".
   답이 하나로 정해지지 않으니 채점자(사람이든 AI든)가 매번 다르게 판정한다.
2. **정답 판정이 "본문에 키워드 포함"** 인데 키워드 검색 축이 정확히 같은 연산을 한다.
   검색기가 채점 기준을 미리 아는 구조다.

### 새 평가셋 v2 — 질문과 합격 기준을 한 몸으로

`data/eval/retrieval_eval_set_v2.json` (20문항). 질문마다 **합격 기준 3개**를 함께 쓴다.

```
질문: 김재섭 위원은 티몬·위메프에 대한 금융감독원의 관리감독 책임을 어떤 근거로 주장했나요?
합격 기준 (셋 다 만족해야 정답)
  1. 발언자가 김재섭이어야 함
  2. 티몬·위메프의 전자금융업 등록 사실을 근거로 들어야 함
  3. 금감원의 관리감독 의무를 주장하는 내용이어야 함
```

기준은 **키워드 매칭으로 환원되지 않는 의미 판정형**으로 쓴다. 이 설계는 HelixOps 프로젝트의
`success_criteria` + 수치 임계값 방식을 참고했다 — 질문 옆에 "무엇을 만족하면 합격인가"가
숫자·조건으로 박혀 있으면 채점자가 헤맬 여지가 없다.

구성: person 5 / topic 4 / comparison 3 / date_based 2 / numerical 2 / quote 2 /
**unanswerable 2**. unanswerable 은 코퍼스에 실재하지 않는 것으로 골랐다 —
법제사법위원회(9개 위원회에 없음), 2024년 기재위(기재위는 2026-01-13부터).

**모범 답안**(`data/eval/모범답안.json`): 관련 발언을 **날짜 상관없이 전부 모아** 하나의
답으로 종합했다(평균 873자, 근거 219건, 최대 14개 날짜). 검색된 조각 하나를 정답으로
내놓지 않는다 — 사람이 "이 답이 기준 3개를 만족하나"를 판단할 수 있어야 하기 때문.

### 검증 — AI 3회 + 사람 1회

| 라운드 | 결과 |
|---|---|
| 1차(Opus) | 살림 8 / 고침 11 / 버림 1 — 수집이 초기 날짜에 갇힌 것 발견 |
| 2차(Opus) | 살림 12 / 버림 1 — **"근거부족" 7건이 전부 파이프라인 탓**임을 확인(코퍼스엔 답이 있었다) |
| 3차(Opus) | 살림 14 / **버림 0** — 남은 결함이 "날짜·화자 오귀속" 한 종류로 수렴 |
| 자동 수정 | 날짜 검증기 + gpt-4o 재생성 → **20/20** |
| **사람 검토** | **20/20 통과** (문항별 합격기준 3개를 개별 확인) |

**과정에서 배운 것**

- **적대적 프레이밍은 거짓 양성을 만든다.** 2차 검증을 Fable·Opus 에 "틀린 걸 찾아라"로
  지시했더니 27건을 지적했는데, 사람이 15건을 직접 보니 **1차 심판(gpt-4o-mini)이 맞았다.**
  POL-7 에서 실측한 "LLM 상호 일치 67.5퍼센트 vs 사람 일치 27.5퍼센트"가 재현된 것이다.
  이후 모든 점검 지시문에 "좋은 것은 좋다고 하라"를 명시했다.
- **정보를 버린 뒤 판정하면 안 된다.** 검수 큐를 280자로 잘라놓고 2차 판정을 시켰더니
  1차 심판(700자)보다 **적은 정보로** 판단하는 역전이 생겼다. 요약(80자)으로 판정시킨
  시도도 같은 실패였다. 판정은 전문으로 해야 한다.
- **`LIMIT` 에 `ORDER BY` 가 없으면 물리 순서로 잘린다.** 근거 수집이 초기 날짜에
  갇혀 "근거부족" 7건을 만들었다. 관련도 정렬로 교체 후 0건.
- **위원회 조건이 "소속"인지 "발언 장소"인지 모호했다**(사용자 지적). 김남근은 국토위
  소속인데 정무위 발언 1,155건 — 임기 중 상임위 이동을 `members_22.json` 이 추적하지
  않는다. 9곳을 "○○위원회 **회의에서 나온** 발언"으로 통일했고, 정당도 같은 스냅샷
  한계라 기준 문구에 그 사실을 적어 넣었다.

### 새 채점기 — `scripts/retrieval_eval_v2.py`

라벨을 미리 만들어 두고 채점기는 **조회만** 한다(결정적·고속, 검색 로직과 판정 로직 분리).

- **strict**(rel>=2) / **loose**(rel>=1) 병기 — strict 가 주지표
- **R@5**(qa 모드) / **R@10**(report 모드) — `MODE_CONFIG` 의 limit 과 맞춘 값
- **nDCG@5·@10 신규** — 리랭커는 "찾은 것을 위로 올리는" 일이라 등급 지표 없이는 못 잰다.
  IDCG 를 검색 결과가 아니라 **라벨 전체**에서 만든다(결과를 정렬해 IDCG 를 만드는 흔한
  구현 오류를 피함 — 그러면 항상 1.0 에 가까워진다)

### 첫 측정 결과

| 모드 | strict R@5 | loose R@5 | MRR@10 | nDCG@5 |
|---|---|---|---|---|
| keyword | 0.556 | 0.833 | 0.313 | 0.285 |
| vector | 0.222 | 0.778 | 0.111 | 0.165 |
| **hybrid** | **1.000** | 1.000 | **0.741** | **0.705** |

**`R@5 = 1.000` 은 의미가 약하다.** "상위 5개에 정답이 하나라도 있나"인데 문항당 정답이
평균 6개라 포화된다. **옛 R@5 0.983 이 높았던 것도 동어반복 때문만이 아니라 이 지표가
원래 잘 포화되기 때문이다.** 진짜 정보는 **nDCG@5 0.705** — 정답을 찾긴 하는데 3~4위에
파묻히는 경우가 남는다는 뜻이고, 개선 여지가 정확히 거기다.

풀 편향 점검: 세 모드 모두 상위 5개의 94~100퍼센트가 라벨을 갖고 있어, 라벨 없어서
0점을 맞은 것이 아님을 확인했다.

### 리랭커 재판정 — 채택은 유효했다

"리랭커 채택" 결정은 고장 난 자로 내린 것이라 새 자로 다시 쟀다.

| 구성 | MRR@10 | nDCG@5 | 질의당 비용 |
|---|---|---|---|
| A 리랭커 OFF | 0.369 | 0.326 | 0 |
| B ON (기존 지시문) | 0.708 | 0.663 | $0.00114 |
| **C ON + 지시문 개선** | **0.741** | **0.705** | $0.00114 |
| D C + 스니펫 1200자 | 0.710 | 0.653 | $0.00181 |

**리랭커는 순위 품질을 2배로 만든다**(0.326 → 0.705). 그리고 찾는 능력(loose 0.944 →
1.000)보다 **순서 바로잡기**에서 훨씬 크게 기여한다 — 제 일을 하고 있다는 뜻이다.

**지시문 개선(C)은 비용 0으로 +7퍼센트포인트.** 넣은 규칙: 사건·사안이 다르면 뒤로
(티메프 질문에 전세사기 발언이 오던 유형), 의사진행·절차 발언 강등, 질문 유형별 우선순위.

**스니펫 확대(D)는 반증됐다.** "더 많이 보여주면 낫겠지"는 상식적 직관이지만 재보니
점수는 내려가고 비용만 1.6배가 됐다. listwise 로 후보 30개를 한 번에 주는 구조라
후보당 길이를 늘리면 총 입력이 커져 판단이 흐려지는 것으로 보인다. 되돌리고 근거를
코드 주석에 남겼다 — **자가 없었으면 그냥 넣었을 변경이다.**

### 남은 것

- **`unanswerable` 1/2** — 법사위 질문(n019)이 리랭커 ON/OFF 무관하게 실패한다.
  코퍼스에 없는 위원회인데 근거를 가져온다. 리랭커가 아니라 **`pre_gate` 영역**이며,
  감사에서 지적된 "pre_gate 가 사실상 발화하지 않는다"와 같은 항목이다.
- **모델 교체 실험 (2단계 미완)** — `gpt-5.6-luna` 가 API 에 실재함을 확인했다.
  리랭커 모델만 교체해 재측정하는 것이 다음 순서. 한 번에 한 변수만 바꾼다.
- **답변 평가(answer_eval)에도 같은 점검이 필요하다.** 검색 잣대는 고쳤지만 답변 잣대
  (75문항, 89.3퍼센트)는 아직 자동채점 과잉감점 전례가 있는 상태다.
- report 모드 전용 항목(구조 준수·시간순·이슈 주입)은 검색 평가로 측정 불가 — 답변 평가 몫.

### 한계 (수치 인용 시 반드시 병기)

- **pooled recall** — 라벨은 5개 검색 축이 찾아온 후보에만 붙는다. 어느 축에도 안 걸린
  근거는 영원히 라벨이 없다. 이 값은 recall 이 아니다.
- 라벨은 **LLM 심판이 매긴 것**이다. 사람은 모범답안 20건을 검토했을 뿐 라벨 879쌍을
  전수 검증하지 않았다.
- 20문항은 자체 작성이며 공인 벤치마크가 아니다.

---

## 답 없는 질문 판정 + 감사 잔여 + 리포트 구조 계약 (2026-08-07)

배포 전 마지막 정리. A(답 없는 질문) → C(감사 잔여) → B-축소(리포트 구조) 순.

### A. 답 없는 질문을 못 막던 원인은 `pre_gate` 가 아니었다 (`e0a329c`)

`unanswerable` 2문항 중 1문항만 막히고 있었다. `grounding.pre_gate` 를 의심했으나
진단해 보니 **질문 해석 단계에서 필터가 안 걸려 검색이 0건이 될 기회 자체가 없었다.**
`pre_gate` 의 `if not hits: return "NONE"` 은 멀쩡했다.

| | 뽑힌 필터 | 실제 결과 | 원인 |
|---|---|---|---|
| n019 법사위 | 없음 | 행안위 발언 | `COMMITTEE_MAP` 에 법사위 없음 |
| n020 2024 기재위 | 기재위만 | 2026년 자료 | 날짜 정규식이 `년+월` 이상만 인식 |

- `_DATE_YEAR_RE` 추가 — 연도 단독("2024년")을 그 해 전체 범위로. 월·일이 붙으면
  기존의 더 좁은 규칙이 이긴다.
- `ABSENT_COMMITTEES` 추가 — 코퍼스에 없는 상임위(법사위·교육위·환노위 등)를
  이름은 인식하되 존재하지 않는 위원회로 필터. 검색 0건 → `pre_gate` "NONE".

**결과: unanswerable 1/2 → 2/2.** 두 변경 모두 n019·n020 에만 걸리는 것을 확인해
나머지 18문항의 검색 입력이 동일함을 보장했다 (동시 측정된 strict R@5 0.944 는
이 수정과 무관한 `gpt-5.6-sol` 의 실행별 흔들림 — `temperature=0` 불가).

### C. 감사 잔여 4건 (`a00ff08`)

가장 심각했던 건 **어제 리랭커를 교체하면서 되살아난 결함**이다.

- **비용 단가가 모델별로 안 나뉨.** `est_cost_usd` 는 `guard` 의 일별 상한($1)이
  읽는 장부인데, 단가 상수가 하나뿐이라 재순위 토큰에도 답변 모델(gpt-4o-mini)
  단가를 곱했다. 재순위를 `gpt-5.6-sol`(추론형, 출력 ~5배)로 바꾸자 장부가
  실지출보다 작아졌다 — 2026-08-04 에 "재순위 비용 누락"으로 고쳤던 것과 같은 결함이
  형태만 바꿔 재발. `PRICES` 표 + `_price`/`_cost` 로 분리.
  미등록 모델은 **보수적 폴백**(5/20 per 1M): 과소추정은 상한을 무력화해 청구서로
  돌아오지만(되돌릴 수 없음) 과대추정은 질의가 일찍 거절될 뿐이다(되돌릴 수 있음).
  실측 질의 1건 0.002 → 0.046 USD. **`gpt-5.6-sol` 실단가 확인 후 `PRICES` 에 넣을 것**
  (현재 상한이 과하게 일찍 걸린다 — 하루 약 21질의).
- `/meetings`·`/speakers` 페이지네이션 정렬 불안정 — 동점 구간 순서가 미정이라
  `LIMIT/OFFSET` 에서 행이 중복·누락된다. `source_id`·`name` 타이브레이커 추가.
- `reranker` 만 `print()` 사용(backend 유일) → `logging`. 재순위가 죽어도 검색은
  폴백으로 계속 도므로(nDCG@5 0.778→0.326) 로그가 유일한 신호다.
- `.env.example` 이 실측과 불일치("선택·기본 OFF·gpt-4o-mini") → 기본 1,
  `RERANKER_MODEL`·`RERANKER_EFFORT` 명시.

### B-축소. 리포트 모드 **구조 계약** 검사 신설 (`scripts/report_eval.py`)

리포트 모드는 지금까지 한 번도 측정된 적이 없었다. 내용 정확성까지 재려면 답변
평가셋(`answer_eval_set.json`)이 검색 평가셋과 같은 재작성을 받아야 하는데 —
그건 배포를 미뤄야 하는 규모다. 그래서 **형식 준수만** 기계적으로 검사한다
(LLM 심판 없음 → 결정적·저비용·반복 가능). 내용 규칙은 `verification.py` 담당이라
겹치지 않는다.

검사 9종: 필수 섹션·순서, 인용 유효성, 문단 인용, 총평 금지, 발언 발췌,
꼬리 거절문, 시간순, 근거 활용률.

**발견한 결함: `## 개요` 에 인용 번호가 하나도 없었다 (18문항 전부).**
개요만 읽는 독자가 가장 많은데 그가 근거를 확인할 방법이 없었다. 출처 없는
`(코퍼스 분석 기준)` 꼬리표까지 붙었다. report 프롬프트에 개요도 `[n]`을 달도록
명시 → **전 항목 통과 0/18 → 17/18**, 근거 활용률 34% → 39%.
(남은 1건은 같은 증상이나 매번 나오진 않는 모델 흔들림)

### 채점기 자체를 테스트한다 (`tests/test_report_eval.py`, 10건)

이 프로젝트는 채점기를 검증 없이 믿다가 두 번 데였다 — 옛 `R@5 0.983`(동어반복),
그리고 이번 C4 첫 버전(문장 단위로 세어 멀쩡한 리포트를 27~60%로 찍음. 리포트가
아니라 검사기가 틀렸다). 합격·불합격 양쪽을 다 고정했다. 특히 "문단 끝에 인용을
붙이는 정상 형태"를 통과로 못 박아 과검출 회귀를 막는다.

### 한계

- 이 점수는 **형식 준수**다. 리포트 내용의 정확성은 아래 절에서 별도로 측정했다.
- 답변 평가셋(75문항, 89.3%)은 검색 평가셋이 받은 검토를 아직 안 받았다.
  그 수치를 품질 근거로 쓰지 말 것.

---

## 리포트 내용 정확성 + 답변 모델 교체 + 키워드 IDF (2026-08-07 오후)

### 기존 답변 채점표(89.3%)가 재던 것 — 정답 여부가 아니었다

`answer_eval_*` 의 4기준은 ①근거 블록 밖을 지어냈나 ②`[n]` 이 맞는 근거를 가리키나
③여야·정당 분류가 맞나 ④근거 없을 때 거절했나 — **넷 중 어느 것도 "질문에 대한
답이 맞았나"를 묻지 않는다.** 검색이 엉뚱한 회의록을 가져와도 그걸 얌전히 요약하고
`[n]`만 잘 달면 전원 통과다. 답변이 **검색 결과에 대해** 채점될 뿐 **사실에 대해**
채점되지 않는다 — 옛 `R@5 0.983` 과 같은 종류의 동어반복.
부수 문제: `must_not_checklist` 가 75문항 전부 비어 있는데(코드 주석은 이를 "채점
정확도의 핵심"이라 부른다), 사람 검수는 21/75 뿐이다.

→ `scripts/report_content_eval.py` 신설. **정답지(`모범답안.json`)를 따로 둔다.**
정답지는 검색과 무관하게 만들어졌고 사람이 20/20 검토했으므로, 검색이 틀리면 떨어진다.

### 답변 모델 교체 — terra 채택 (18문항, 정답지 대비)

| 모델 | 통과 | 환각 | 평균 길이 |
|---|---|---|---|
| `gpt-4o-mini` | 7/18 (38.9%) | 19건 | 998자 |
| **`gpt-5.6-terra`** | **13/18 (72.2%)** | **5건** | **1858자** |
| `gpt-5.6-sol` | 12/18 (66.7%) | 8건 | 2090자 |

terra 가 sol 보다 더 맞히고, 환각이 적고, 더 짧다(=싸다). 미통과 5건은 사람이 검수해
심판 판정이 옳음을 확인했다. 옛 모델이 특히 못하던 곳은 숫자 0/2·인용 0/2·날짜 1/2
— 수치를 틀리게 적고 인용문을 바꿔 쓰던 문제로, terra 는 전부 만점.

**주의**: `gpt-5.6` 계열 단가는 미확인이라 `answer.PRICES` 의 보수적 폴백(5/20 per 1M)
이 적용 중이다. 실측 질의 1건이 0.046 USD 로 잡혀 `$1/일` 상한이 하루 21질의로
조인다. 실단가 확인 후 `PRICES` 에 넣을 것.

### 비교 질문이 3모델 모두 0/3 이던 진짜 원인 — 생성이 아니라 검색

`n010`(티메프 여야 대비) 상위 30건 중 티메프·티몬·위메프가 **실제로 든 것은 6건**뿐.
1위는 **국방위 안보 발언**이었다 — '사태·정부·책임·여당·야당' 5개를 갖고 있어서.

원인: 키워드 점수가 "토큰 하나 맞으면 +1" 이라 모든 단어가 같은 값이었다.
코퍼스에서 **"정부"는 30,696건(7.30%), "티메프"는 99건(0.02%) — 310배 차이**인데
둘 다 +1. 비교 질문은 '여당·야당·시각·범위' 등 흔한 말이 많아 이 희석이 가장 심하다.

→ **IDF 가중치 도입** (`search_keyword.token_weight`, 0.25~3.0).
질의 때 세면 흔한 토큰 하나에 ~400ms 라 미리 계산해 `data/token_df.json` 에 둔다
(`scripts/build_df_table.py`, 1200토큰, 5분). 표에 없는 토큰은 **드문 것으로 간주** —
드문 쪽이 안전한 기본값이다(고유명사 질문이 이 도메인의 다수).
배포 코퍼스에 trgm 인덱스가 없다는 점에서도 사전 계산이 맞다.

| | 전 | 후 |
|---|---|---|
| 상위 30건 중 티메프 실제 포함 | 6건 | **20건** |
| 상위 10건 위원회 | 4개 섞임 | 정무위 6 · 산자중기위 4 |

### 함정 — "고쳤더니 점수가 떨어졌다"

IDF 적용 직후 nDCG@5 가 **0.751 → 0.547** 로 떨어졌다. 되돌릴 뻔했으나 원인은
**채점표가 낡아서**였다. qrels 라벨은 옛 검색이 풀링한 후보에만 붙어 있고
채점기는 `labels.get(chunk_id, 0)` — **라벨 없으면 무조건 0점**이다.
실측: 상위5 중 **39%(35/90)가 무라벨**, `n010` 은 5/5 전부(눈으로 보면 전부 정무위
티메프 발언인데도).

**잘 고칠수록 점수가 떨어지는 구조.** 이것이 문서에 적어둔 "pooled recall" 한계의
실체다 — 추상적 주의사항이 아니라 실제로 좋은 수정을 되돌릴 뻔했다.
→ 풀 재생성 + `--resume` 로 신규 227쌍만 채점(22초, $0.025).

| | 아침 | 라벨 갱신 전 | **갱신 후** |
|---|---|---|---|
| strict R@5 | 0.944 | 0.833 | **1.000** |
| MRR@10 | 0.868 | 0.719 | **0.907** |
| nDCG@5 | 0.751 | 0.547 | **0.755** |
| 비교 질문 strict@5 | 0.67 | 0.33 | **1.00** |

### 여야 분류 규칙 충돌 — 프로그램이 억울하게 떨어지고 있었다

`n012` 합격 기준은 "최신 당적 기준 진영 대비", 시스템 지시문은 "[정당(당시 여야)]로
분류". **서로 반대**였고 프로그램은 시킨 대로 하고 떨어졌다.

근본 원인: 코퍼스 기간에 정권교체(2025-06-04)가 들어 있어 **같은 사람·같은 정당이
날짜에 따라 여당↔야당으로 뒤집힌다** (최형두 2024-07-25 여당 → 2025-08-26 야당).
여러 날짜에 걸친 질문에서 이를 묶는 축으로 쓰면 한 사람이 양쪽에 들어간다.

→ **묶는 축은 정당**, 여야 지위는 날짜와 함께 덧붙이도록 지시문 수정.
위성정당(국민의미래·더불어민주연합)은 모정당과 합친다.
`party.party_bloc()` 신설 — 같은 원칙을 검색의 진영 균형 배분에도 적용
(`search_hybrid._balance_by_side`, 위원회 균형과 같은 구조).

### 숨김용 평가셋으로 일반화 검증 (`heldout_*`)

18문항을 보며 고치면 그 18문항은 당연히 오른다. 이 프로젝트는 이미 같은 함정에
빠진 적이 있다(감사 항목: "검증층이 63문항에 과적합 — 같은 63문항으로 만들고 같은
63문항으로 검증"). 그래서 **고치기 전에** 비교 질문 5개를 새로 만들어 손대지 않고
두었다(`scripts/build_gold_answers.py`, 사람 검토 완료).

정답지 근거는 **시스템 검색을 쓰지 않고** 직접 SQL 로 훑는다 — 시스템이 못 찾은
근거가 정답지에도 없으면 채점이 자화자찬이 된다. 수집 중 발견: 적중 수로만 줄
세우니 발언량 많은 진영이 독식(원전 질문 여22:야3) → 축별로 나눠 담아 20:20 균형.
...
**한 번도 보지 않은 문제에서 올랐다 — 외운 것이 아니라 일반화됐다.**
(단, 여야 규칙 수정은 지시문이라 껐다 켤 수 없어 양쪽 모두 적용된 상태다.
이 비교는 IDF 효과를 분리해 보여준다.)

### 최종 (18문항, terra + IDF + 규칙 수정)

    전체 통과 14/18 (77.8%) · 합격 기준 52/54 (96.3%) · 환각 3건
    유형별: person 5/5 · quote 2/2 · numerical 2/2 · date_based 2/2
            topic 2/4 · comparison 1/3

아침 대비 환각 5건 → 3건, 인물 3/5 → 5/5. 비교 질문은 검색이 1.00 이 됐으나
생성 단계에서 여전히 1/3 — 남은 실패는 발언자 오귀속과 "감액 의견"을 "감액 완료"로
쓰는 유형이다(모델 한계에 가깝다).

### 한계

- 심판은 LLM 1차 판정이다. 실제로 놓친 사례가 있다 — terra 이전 측정에서 회의록에
  없는 "정책금리 인하"를 지어낸 것을 심판이 모순으로 잡지 못했다.
- 숨김용 5문항은 비교 유형만 다룬다. 다른 유형의 일반화는 검증되지 않았다.
- IDF 표는 표본 3만 청크에서 뽑은 상위 1200토큰이다. 코퍼스가 바뀌면 재생성해야 한다.

### qa 모드도 측정 — 리포트보다 낫다

두 모드 중 리포트만 재고 있었다. 같은 채점기로 qa(top-5·700토큰)를 재니:

| 18문항 | qa | report |
|---|---|---|
| 통과 | **16/18 (88.9%)** | 14/18 (77.8%) |
| 환각 | **1건** | 3건 |
| topic | **4/4** | 2/4 |

짧게 답할수록 지어낼 여지가 줄어든다. 남은 실패 2건은 둘 다 비교 질문(n010·n012)
으로, report 모드의 실패 지점과 같다 — 모드가 아니라 유형의 문제다.

---

## 배포 방어선 잔여 5건 (2026-08-07)

감사 목록에서 "배포하면 사용자에게 바로 보이는" 것들을 처리했다.

- **에러 바운더리 없음** → `ErrorBoundary.jsx` 신설. React 는 렌더 예외를 잡지
  않으면 트리 전체를 언마운트한다 — 컴포넌트 하나가 예상 못 한 응답을 만나면
  **화면이 통째로 백지**가 되고 사용자는 새로고침 말고 할 게 없다.
  본문만 감싸 헤더·탭은 살려 두고(다른 화면으로 이동 가능), `resetKey={tab}` 으로
  탭 이동 시 자동 복구. **실제로 Hero 를 일부러 던지게 해서 확인**했다 — 백지 대신
  안내가 뜨고, 탭을 옮기니 복구됐다.
- **`/feedback` 무인증** → 예전에는 `query_id` 만 알면 누구나 어느 평가든 덮어썼다
  (표본 3건이라 한 번 오염되면 통계가 뒤집힌다). 로그인 사용자의 질의는 그 사용자만,
  익명 질의는 최초 1회만. 실패는 404 로 통일 — "남의 것"이라 알려주면 존재 여부가 샌다.
- **`/health` 가 매 호출 42만 행 `count(*)` 2회** → `pg_class.reltuples` 근사.
  프론트가 페이지 로드마다 부르는 곳이고 행 수는 참고값이다. 통계가 없으면(-1)
  실집계로 떨어진다.
- **브라우저 뒤로가기 무효** → `replaceState` 만 써서 히스토리가 안 쌓였다.
  `pushState` + `popstate` 리스너로 교체. **브라우저로 실제 확인**: 탭 2번 이동 후
  뒤로가기 두 번에 `?tab=actor → ?tab=issues → (질의)` 로 주소·화면이 함께 복원.
  구현 중 함정 하나 — "주소가 같으면 return" 을 popstate 플래그 해제보다 앞에 두면
  뒤로가기 직후 플래그가 남아 **다음 탭 클릭이 통째로 무시된다**. 실측으로 발견해
  해제를 맨 앞으로 옮겼다.
- **CORS 기본값이 개발용** → 미설정 시 조용히 localhost 로 가던 것을 경고 로그와
  함께 간다. 런북에 적혀 있어도 사람은 빠뜨리고, 그때 프론트는 전부 CORS 오류인데
  서버 로그엔 아무 신호가 없었다.

### 의존성 취약점 정리 (배포 직전)

푸시 후 CI 의 `pip-audit` 단계가 빨간불이었다. **CI 판정 자체는 success** 다 —
감사 때 `continue-on-error` 로 바꿔 빌드를 막지 않게 해둔 설계가 의도대로 동작한
것이다(단계는 빨갛고 작업은 통과). 다만 보고된 취약점은 진짜였다.

| 패키지 | 전 | 후 | 건수 |
|---|---|---|---|
| `starlette` | 0.38.6 | 1.4.1 | 7 |
| `PyJWT` | 2.10.1 | 2.13.0 | **6 (인증 핵심)** |
| `requests` | 2.32.3 | 2.33.0 | 2 |
| `lxml` | 5.3.0 | 6.1.0 | 1 |

`starlette` 는 `fastapi` 가 끌고 오므로 `fastapi` 0.115.0 → 0.141.1 로 함께 올렸다.
메이저에 가까운 도약이지만 **152개 테스트 전원 통과**.

부수 발견 둘:

- **로컬 설치본이 `requirements.txt` 와 어긋나 있었다** (`uvicorn` 0.40 vs 적힌
  0.30.6, `pydantic` 2.12.5 vs 2.9.2). CI 는 이 파일로 새로 설치하므로, 어긋나 있으면
  **로컬 테스트가 CI 를 대변하지 못한다.** 실제 설치본에 맞춰 고정했다.
- **짧은 `JWT_SECRET` 이 조용히 통과하고 있었다.** PyJWT 2.13 이 "HMAC 키가 32바이트
  미만" 경고를 내기 시작해 드러났다(RFC 7518 §3.2). 자동 생성 키는 64자라 안전한데
  사람이 `mysecret` 같은 것을 넣으면 **미설정보다 나쁜 상태**가 되면서도 아무 신호가
  없었다. 32바이트 미만이면 error 로그와 함께 무작위 키로 대체한다 — 배포는 뜨되
  약한 키로는 돌지 않는 fail-safe. 실측 확인: 8바이트 입력 → 64바이트로 대체,
  32바이트 입력 → 그대로.

`pydantic-settings` 취약점도 검출됐으나 `langchain-community` 소속으로 이 프로젝트
의존성이 아니다(전역 환경에 섞여 있던 것).

**한계**: 윈도우에서는 `pip-audit -r` 이 한글 주석 때문에 파일을 못 읽는다
(cp949 로 디코딩 시도). 로컬 검증은 설치 패키지 대상(`pip-audit` 인자 없이)으로
해야 한다. CI(리눅스)는 영향 없다.

---

---

## 4단계-B 실배포 완료 (2026-08-07)

계정 연결부터 스모크까지 끝냈다. 주소는 README "실제 배포 주소" 절에 있다.

| 조각 | 선택 | 결정 근거 |
|---|---|---|
| 프론트 | Vercel Hobby | Root `frontend`, preset **Vite** |
| 백엔드 | Render Free, **싱가포르** | 무료 리전 5곳 중 서울 DB 에 가장 가깝다 |
| DB | Supabase 서울 (139MB/500MB) | Session Pooler 5432 |

리전 선택은 실측으로 이득이 확인됐다 — DB 만 쓰는 `/actors/김윤` 이 로컬(4.9초)보다
배포본(2.5초)에서 빨랐다. **리전은 생성 후 변경 불가**라 만들기 전에 정해야 한다.

### 콘솔에서 시간을 잡아먹은 함정 셋

1. **Render 환경변수 편집이 조용히 안 먹는다.** `BACKEND_CORS_ORIGINS` 를 고치고
   "Save, rebuild, and deploy" 를 눌렀는데 저장된 값은 이전 값 그대로였다.
   재배포는 성공으로 끝났다 — **아무것도 바뀌지 않은 재배포**다. 값을 눈으로
   확인하기 전까지는 저장됐다고 믿을 수 없다. 행을 지우고 새로 만들어 키보드로
   직접 입력하면 반영된다.
2. **Render Instance Type 기본값이 Starter($7/월)** 이다. Free 는 직접 골라야 한다.
3. **Vercel 이 백엔드까지 배포하려 든다.** preset 기본값 `Services` 가 `frontend`(Vite)와
   `backend`(FastAPI)를 둘 다 감지한다. `Vite` 로 바꾸지 않으면 FastAPI 가 중복 배포된다.

### 배포 스모크 — 13종, 결함 2건

전부 실제 배포 주소 상대로 쟀다. 비용 $0.26 (일별 상한 $3).

| 항목 | 결과 |
|---|---|
| 일반 요청 제한 60/분 | **정확히 61번째에서 429** |
| LLM 제한 5/분 | 5회 통과 후 **6번째 429** (0.4초 즉시) |
| 동시 요청 | 5건 2.5초 / 10건 3.9초 — 전부 200 |
| 쟁점 24개 전수 (72요청) | 실패 0 · 5초 초과 0 |
| 인증 7종 | 가입·로그인·위조토큰 401·중복가입 409 |
| 평가 소유권 3종 | 없는 query_id 404 · UUID 아님 422 |
| 답 없는 질문 3종 | 전부 "확인할 수 없습니다" — 지어내지 않음 |
| 모바일 375px · 뒤로가기 | 정상 |

**동시 처리는 문서보다 낫다.** 알려진 한계 3번은 "실질 동시 2~3건"이라 적혀 있는데
검색 엔드포인트는 10건 동시에도 전부 200 이었다. 커넥션을 1개만 쓰기 때문이다
(2개 쓰는 건 `/query`). 문서가 `/query` 기준임을 밝히지 않아 실제보다 비관적으로 읽힌다.

**돈 나가기 전에 끊는다** — 검색 0건인 질문은 LLM 을 아예 호출하지 않고 1.6초에
거절했다(비용 $0). 방어선이 비용 발생 지점 앞에 있다는 실측 증거다.

### 결함 2건

**① 공백만 입력하면 502.** `Field(min_length=2)` 를 공백 2칸이 통과해 빈 질문이
임베딩 API 까지 흘러갔고, 사용자는 `502 임베딩 호출 실패: BadRequestError` 라는
내부 오류를 봤다. `StringConstraints(strip_whitespace=True)` 로 교체 — 잘라내기가
길이 검사보다 먼저 돌아 자연히 걸린다. 테스트에 **과잉 차단 방지**(앞뒤 공백은
잘라내되 내용이 남으면 통과)를 함께 고정했다.

**② 첫 화면이 방문자와 다른 코퍼스를 잰 숫자를 띄우고 있었다.** 두 번 밟았다.

- 1차: `검색 정확도 98.3%` 는 **폐기한 잣대(R@5)** 의 값이었다. 더 나쁜 건 바로 위
  주석이 "새 평가셋으로 다시 쟀다"고 적혀 있었다는 것 — 주석만 갱신되고 값은
  그대로였다. **주석이 보증해 주는 바람에 더 오래 살아남았다.**
- 2차: 그래서 실측값 `18/18` 로 바꿨는데, 그건 **전체 코퍼스 로컬 측정**이었다.
  방문자가 쓰는 건 축소본이다. "숫자가 낡았나"만 보고 **"무엇을 잰 숫자인가"를
  묻지 않았다** — 고치던 것과 같은 종류의 실수다.

같은 평가셋을 두 코퍼스에 각각 돌린 결과:
...
**원인은 검색 성능이 아니라 근거의 부재다.** 라벨된 핵심 근거(등급2) 142개 중
축소본에 73개만 있다(48.6% 누락). 누락률 높은 문항이 점수 낮은 유형과 그대로
겹친다(n013 87.8%·n014 96.4%·n018 90.9% → date_based 0.50·person 0.80).
축소본 점수만 보고 "검색이 나빠졌다"고 오독하면 엉뚱한 데를 고치게 된다 —
**근거 존재 여부를 세는 것이 진단의 핵심이었다.**

화면은 `15/18` 을 앞에, `전체 데이터로는 18/18` 을 뒤에 뒀다. 순서를 뒤집으면
폐기 수치를 띄우던 것과 같은 거짓이 된다.

### 재순위 사고 폭주 (신규 방어)

배포 검증 중 같은 프롬프트(입력 8,222토큰 동일)를 두 번 호출했는데 출력이
**244 vs 29,132 토큰**으로 갈렸다. 폭주한 쪽은 질의가 25초 → 125초, 비용
$0.040 → $0.076.

처음엔 Render 무료(0.1 CPU) 탓인 줄 알고 로컬에서 같은 질문을 돌렸다. 25초였다.
그런데 **입력 토큰이 동일**했다 — 같은 프롬프트다. 환경 차이가 아니라 추론형
모델의 사고량이 호출마다 튀는 것이고, `reasoning_effort=low` 는 이 꼬리를 막지
못한다. `RERANKER_MAX_TOKENS=2000` 신설(실측 정상 출력 244·402 의 5배 여유).
상한에 걸리면 원순위 폴백 + 경고 로그. 실호출로 양쪽 경로를 확인했다.

### 배포 코퍼스 확대 시도 — 세 번 틀리고 도착한 결론

축소본에 근거의 절반이 없다는 것을 알고, 남는 용량(139MB/500MB)으로 넓히려 했다.
**결론부터: 무료 한도 안에서 이 방향은 거의 소진됐다.** 도달 과정이 기록할 값어치가 있다.

**측정 1 — 근거는 어디 있나.** 누락 근거의 95.1%가 쟁점 turn 이 있는 같은 회의 안.
다만 거리가 넓게 퍼져 있다: ±5 로 26%, ±20 으로 51%, **±60 까지 가야 100%**.
인접 창을 조금 넓히는 것으로는 회수가 안 된다는 뜻이다.

**측정 2 — 인덱스가 용량을 먹는다.** HNSW 를 유지하면 500MB 를 다 써도 ±1(회수
49.3%)이 한계다. 인덱스를 빼면 같은 용량에 ±5(60.3%)가 들어간다.

**측정 3 (틀림) — 인덱스 없는 순차 스캔 비용.** 로컬 420,378행에서 0.51초를 재고
50,726행으로 환산해 0.06초라고 판단했다. 실제 배포에서는 **벡터 검색 3.7초 →
26.4초, 하이브리드 10.2초 → 28.4초**였다. 개발 PC(빠른 디스크·충분한 RAM)와
Supabase 무료(공유 CPU·작은 RAM)는 290MB 벡터를 캐시할 수 있느냐에서 갈린다.
**배포 환경이 아닌 곳에서 잰 숫자를 배포에 적용한 것** — 같은 날 화면 수치에서
지적했던 실수를 스스로 반복했다. 이번이 세 번째다.

경로에서 나온 결함 둘:

- **`--no-index` 가 인덱스를 안 지웠다.** 생성을 건너뛸 뿐인데 `schema.sql` 이
  무조건 만들고 `TRUNCATE` 는 인덱스를 지우지 않는다. 재적재하면 행이 들어가며
  기존 인덱스가 그대로 다시 찬다. 결과 **879MB — 무료 한도의 176%**.
- **보정계수를 잘못된 구성에 적용했다.** 0.83 은 인덱스 있는 배포본에서 잰 값인데
  없는 구성에 그대로 썼다(362MB 예상 → 실제 504MB). 벡터는 TOAST 로 나가 실측
  행단가가 추정보다 크다(6.5KB 가정 → 실측 8.4KB).

**최종 적용: ±1 + HNSW, 18,001청크, 327MB/500MB.**

| | 청크 | 용량 | strict@5 | nDCG@5 | 하이브리드 |
|---|---|---|---|---|---|
| 확대 전 | 6,692 | 139MB | 15/18 | 0.468 | 10.2초 |
| **현재** | **18,001** | **327MB** | **15/18** | **0.522** | **5.2초** |

**strict@5 는 그대로다.** 2.7배를 넣고도 안 오른 이유가 측정 1이다. 순위 품질과
지연은 개선됐다. 첫 화면의 `15/18` 은 바꿀 필요가 없었다 — 정직하게 적어 둔 수치는
데이터가 바뀌어도 다시 안 고쳐도 된다.

**자동 폴백(choose_scope)을 제거했다.** 애초에 배포본이 씨앗 turn 만 담았던 원인이
그것이다. 인접 ±1 이 옛 한도(350MB)를 넘자 조용히 '인접 제외'로 떨어졌고, 로그에는
선택 결과만 찍혀 **인덱스를 지키려고 근거를 버렸다는 사실**이 드러나지 않았다.
자동 폴백은 실패하지 않는 것처럼 보이게 만들면서 트레이드오프를 사람 눈에서 감춘다.

### 남은 것

- **임베딩 차원 축소(1536 → 512)** — 저장·인덱스가 1/3 이 되어 같은 용량에 3배를
  담을 수 있다. 재생성 비용 약 $0.22. **차원 축소가 검색 품질에 주는 영향을
  배포와 같은 조건에서 먼저 잴 것** — 위 측정 3 의 교훈이다.
- 답변 평가셋 75문항 재실행 (일별 상한을 거의 다 쓴다 — 날을 따로 잡을 것)
- 알려진 한계 3번 문장을 `/query` 기준임이 드러나게 다듬기
- `python-dotenv` 1.0.1 → 1.2.2 (PYSEC-2026-2270)
- Supabase 무료는 7일 무접속 시 프로젝트를 재운다 — 시연 전날 깨울 것
