...
최종 업데이트: 2026-07-09 (POL-5 입장 분석 파일럿+24개 확장·프론트 뷰 → POL-7 eval 설계 착수. 다음: POL-7 구현→사용자 라벨링→POL-5 검증 완료)
...
| POL-5 | 입장(stance) 분석 | 이슈×행위자 입장(찬성/반대/우려/중립) — stance_signals 규칙 vs LLM 판정 **결정 지점**. 모든 입장에 근거 발언 인용 필수 (신뢰 원칙) | 이슈 1개의 행위자별 입장+근거 매트릭스 | 🔶 2026-07-09 (파일럿 medical-reform + 24개 전체 확장 + 프론트 뷰. **미검증** — 하드게이트·POL-7 라벨 잔여) |
| POL-6 | 여야 대립 구도 | 이슈별 여야(발언 시점 기준) 입장 분포 — POL-0 정당 모듈 × POL-5 입장 결합 | "이슈 X의 여야 입장 차이" 질문에 근거 있는 구도 응답 | ⬜ |
| POL-7 | 분석 eval | 입장 판정 수동 라벨 세트(30~50건) + 타임라인 정합성 검사 — 이후 변경마다 재실행 (마스터 5-5) | 기준선 점수 기록 | 🔶 설계 완료 (2026-07-09, 블라인드 라벨 40건 — POL-5 검증. spec 참조. 계획·라벨링 잔여) |
