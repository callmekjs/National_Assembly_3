...
MAP_VERSION = "s1.1"     # s1.1: index-keyed 출력 — positional 정확길이 계약의 miscount 유실 수정
BATCH_SIZE = 20
_MODEL = "gpt-4o-mini"
...
확신이 없으면 neutral 또는 none 으로. 반드시 아래 JSON 만 출력. 주어진 각 발언 번호 i 마다
정확히 하나의 객체를 넣는다:
{"items": [{"i": 0, "stance": "concern"}, {"i": 1, "stance": "support"}, ...]}"""


def parse_stance_response(content: str, batch_size: int) -> list[str | None] | None:
    """index-keyed 응답 → 길이 batch_size 리스트(각 위치 stance, 미판정은 None).
    구조 자체가 오류면 None(재시도 신호). 과다 인덱스는 무시, 누락 인덱스는 None 으로 흡수.
    이유: positional 정확길이 계약은 LLM 의 개수 miscount(과다·과소 생성)에 취약해 배치
    전체가 유실됨(2026-07-09 디버깅, 배치20 유실 38%). index 정렬로 유실을 발언 단위로 국소화."""
    try:
    ...
    if not isinstance(items, list):
        return None
    out: list[str | None] = [None] * batch_size
    for it in items:
        if isinstance(it, dict):
            i, s = it.get("i"), it.get("stance")
            if isinstance(i, int) and 0 <= i < batch_size and s in _STANCES:
                out[i] = s
    return out
...
def classify_issue(client, issue, judge_model=_MODEL, dry_run=False):
    ...
    for batch in make_batches(turns, BATCH_SIZE):
        ...
        for t, s in zip(batch, result):
            if s is None:          # 해당 발언만 미판정 → 보류 (누락 우선)
                dropped += 1
                continue
